...
import {
  ...
  findUserByUsername, getOrCreateDM, createGroup, sync,
} from "../store"
...
export default function Messenger({ me }: Props) {
  ...
  const refresh = async () => {
    try { await sync(me.id) } catch { return }
  ...
  useEffect(() => {
    if (activeConvo) {
      ...
      const t = setInterval(async () => { try { await sync(me.id); setMsgs(getMsgs(activeConvo.id)) } catch {} }, 1500)
      return () => clearInterval(t)
    }
  }, [activeConvo])
  ...
  async function openDM(friend: User) {
    try { const c = await getOrCreateDM(me, friend); setConvos(getConvosFor(me.id)); openConvo(c) } catch(e) { showNotify(e instanceof Error?e.message:"Could not open chat") }
  }

  async function submit() {
    if (!draft.trim() || !activeConvo) return
    try { await sendMsg(activeConvo.id, me, draft.trim()); setDraft(""); setMsgs(getMsgs(activeConvo.id)); await refresh() } catch(e) { showNotify(e instanceof Error?e.message:"Message failed") }
  }

  async function doSearch() {
    try { await sync(me.id) } catch { showNotify("Server unavailable"); return }
  ...
  async function doFriendRequest(target: User) {
    try { await sendFriendRequest(me, target) } catch(e) { showNotify(e instanceof Error?e.message:"Request failed"); return }
    showNotify(`Friend request sent to @${target.username}!`)
    setSearchQ(""); setSearchResult(null)
  }

  async function doAccept(reqId: string) { await acceptRequest(reqId); await refresh(); showNotify("Friend request accepted!") }
  async function doDecline(reqId: string) { await declineRequest(reqId); await refresh() }

  async function makeGroup() {
    if (!groupName.trim() || groupMembers.length === 0) return
    const c = await createGroup(me, groupName.trim(), groupMembers)
    await refresh()
    openConvo(c)
